  ...
  function initNavbarMagnetic() {
    ...
    links.forEach(function(link) {
      link.addEventListener("mousemove", function(e) {
        ...
        var dx = Math.max(-6, Math.min(6, (e.clientX - cx) * 0.12));
        var dy = Math.max(-3, Math.min(3, (e.clientY - cy) * 0.12));
        link.style.transform = "translate(" + dx + "px," + dy + "px)";
        link.style.textShadow = "0 0 8px rgba(0,188,212,0.25)";
      });
      link.addEventListener("mouseleave", function() {
        link.style.transform = "translate(0,0)";
        link.style.textShadow = "none";
        link.style.transition = "transform 0.4s cubic-bezier(0.34,1.56,0.64,1), text-shadow 0.3s ease";
      });
      link.addEventListener("mouseenter", function() {
        link.style.transition = "transform 0.08s ease, text-shadow 0.15s ease";
      });
    });
  }

  function initPipelineAnimation() {
    var stages = qsa(".pipeline-stage");
    if (!stages.length || !window.gsap) return;
    stages.forEach(function(stage, i) {
      gsap.set(stage, { opacity: 0, scale: 0.85, y: 20 });
      gsap.to(stage, {
    ...
    var connectors = qsa(".pipeline-connector");
    connectors.forEach(function(conn, i) {
      gsap.set(conn, { scaleX: 0, opacity: 0 });
      gsap.to(conn, {
        scaleX: 1, opacity: 0.3, duration: 0.3, ease: "power2.out",
        delay: i * 0.06 + 0.3,
        scrollTrigger: { trigger: conn, start: "top 88%", once: true }
      });
    });
  }

  function initHeroTextReveal() {
    var heroTitle = qs(".hero-main-title");
    if (!heroTitle || !window.gsap) return;
    var spans = heroTitle.querySelectorAll("span");
    if (!spans.length) return;
    gsap.set(spans, { opacity: 0, y: 30, filter: "blur(6px)" });
    gsap.to(spans, { opacity: 1, y: 0, filter: "blur(0px)", duration: 1, ease: "power3.out", stagger: 0.2, delay: 0.3 });
    var subhead = qs(".subhead");
    if (subhead) { gsap.set(subhead, {opacity:0,y:20}); gsap.to(subhead, {opacity:1,y:0,duration:0.8,ease:"power3.out",delay:0.8}); }
    var actions = qs(".hero-actions");
    if (actions) { gsap.set(actions, {opacity:0,y:15}); gsap.to(actions, {opacity:1,y:0,duration:0.6,ease:"power3.out",delay:1.1}); }
    var stats = qs(".stats");
    if (stats) { gsap.set(stats, {opacity:0,y:15}); gsap.to(stats, {opacity:1,y:0,duration:0.6,ease:"power3.out",delay:1.3}); }
  }

  function initParallaxSections() {
    var heads = qsa(".section-head");
    if (!heads.length || !window.gsap) return;
    heads.forEach(function(head) {
      gsap.to(head, { y: -20, scrollTrigger: { trigger: head, start: "top bottom", end: "bottom top", scrub: 1.5 } });
    });
  }
